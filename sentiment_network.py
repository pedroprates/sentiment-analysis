...
class SentimentNetwork:
    def __init__(self, reviews, labels, hidden_nodes=10, learning_rate=0.1, min_count=50, polarity_cutoff=0.01):
        ...
        review_vocab, label_vocab = self.pre_process_data()
    ...
    def pre_process_data(self):
        review_counter = Counter([word.lower() for review in self.reviews for word in review.split(" ")])
        review_vocab = [word for word in review_counter.keys() if review_counter[word] > self.min_count]
        review_vocab = filter(lambda x: np.absolute(self.pos_neg_ratios[x]) > self.polarity_cutoff, review_vocab)

        label_vocab = set({label.lower() for label in self.labels})
    ...
    def train(self):
        ...
        for i in range(len(training_reviews)):
            ...
            self.weights_0_1[review] += self.learning_rate * delta_w_0_1.T
